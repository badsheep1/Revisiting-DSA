...
int main(void) {
  ...
  Matrix matrix1 = newMatrix(5);

  Matrix identity5 = newMatrix(5);

  for (int i = 1; i <= 5; i++) {
    changeEntry(identity5, i, i, 1);
  }

  for (int i = 1; i <= 5; i++) {
    for (int j = 1; j <= 5; j++) {
      changeEntry(matrix1, i, j, (i + j) * (0.23 * i * j));
    }
  }

  Matrix transA = transpose(matrix1);

  Matrix matrix2 = scalarMult(1.61, transA);
  ...
  Matrix product1 = product(matrix1, matrix2);
